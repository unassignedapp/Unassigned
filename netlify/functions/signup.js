...
var makeUsername = () => "ASSIGNED" + Math.random().toString(36).slice(2, 8).toUpperCase();
var makeToken = () => Math.random().toString(36).slice(2) + Math.random().toString(36).slice(2) + Date.now().toString(36);
var toE164 = (raw) => {
...
exports.handler = async (event) => {
  ...
  if (phone) {
    const { data: existing } = await supabase.from("users").select("username, api_token").eq("phone", phone).maybeSingle();
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const { data, error } = await supabase.from("users").insert({ username, api_token: token, phone }).select("username, api_token").maybeSingle();
    if (!error && data) {
      inserted = data;
      break;
    }
    if (error) {
      ...
      if (phone && (msg.includes("duplicate") || msg.includes("unique")) && msg.includes("phone")) {
        const { data: existing } = await supabase.from("users").select("username, api_token").eq("phone", phone).maybeSingle();
        if (existing) return json(200, existing);
      }
      if (msg.includes("duplicate") || msg.includes("unique")) continue;
      return json(500, { error: "signup_failed", detail: error.message });
    }
  }
  if (!inserted) return json(500, { error: "could_not_assign_username" });
