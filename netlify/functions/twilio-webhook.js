...
var OPT_IN = /* @__PURE__ */ new Set(["Y", "YES", "START", "UNSTOP"]);
...
function twiml(message) {
  const inner = message ? `<Message>${String(message).replace(/[<&>]/g, "")}</Message>` : "";
...
exports.handler = async (event) => {
  ...
  if (OPT_OUT.has(word)) {
    ...
    return twiml("You are opted out of Unassigned. No more messages. Reply Y to opt back in.");
  }
  if (HELP.has(word)) {
    return twiml("Unassigned: anonymous link sharing. Reply Y to receive a link, STOP to opt out.");
  }
  if (OPT_IN.has(word)) {
    try {
  ...
  return twiml("Reply Y to receive the link sent to you, or STOP to opt out.");
};
