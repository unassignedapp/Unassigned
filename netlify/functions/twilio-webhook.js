...
var OPT_IN = /* @__PURE__ */ new Set(["Y", "YES", "START", "UNSTOP", "JOIN", "SUBSCRIBE"]);
var OPT_OUT = /* @__PURE__ */ new Set(["STOP", "STOPALL", "UNSUBSCRIBE", "CANCEL", "END", "QUIT"]);
...
function validateSignature(authToken, signature, url, params) {
  ...
  const expected = crypto2.createHmac("sha1", authToken).update(Buffer.from(data, "utf-8")).digest("base64");
...
function publicUrl(event) {
  ...
  const path = event.headers["x-nf-original-pathname"] || event.path;
  return `${proto}://${host}${path}`;
}
exports.handler = async (event) => {
  if (event.httpMethod !== "POST") return twiml("");
  const raw = event.isBase64Encoded ? Buffer.from(event.body || "", "base64").toString("utf8") : event.body || "";
  const params = querystring.parse(raw);
  const signature = event.headers["x-twilio-signature"] || event.headers["X-Twilio-Signature"] || "";
  if (!validateSignature(TWILIO_TOKEN, signature, publicUrl(event), params)) {
  ...
  if (OPT_OUT.has(word)) {
    try {
      await supabase.from("consent_registry").upsert({ phone_number: from, status: "opted_out" }, { onConflict: "phone_number" });
      await supabase.from("held_links").delete().eq("phone_number", from);
    } catch (err) {
      console.error("stop_failed", err.message);
    }
    return twiml("You are opted out of Unassigned. No more messages will be sent to this number. Reply Y to opt back in.");
  }
  if (HELP.has(word)) {
    return twiml("Unassigned: person-to-person link sharing. You receive a link only after you reply Y to consent. Msg & data rates may apply. Help: soft-treacle-9a92e4.netlify.app/consent.html Reply STOP to opt out.");
  }
  if (OPT_IN.has(word)) {
    try {
      ...
      for (const row of held || []) {
        try {
          ...
          console.error("release_failed", row.id, err.message);
        }
      }
      if (deliveredIds.length) await supabase.from("held_links").delete().in("id", deliveredIds);
      return twiml("");
    } catch (err) {
  ...
  return twiml("Unassigned: Reply Y to consent and receive the link shared with you, or STOP to opt out.");
};
