...
// netlify/functions/seal-and-send.js
...
var TWILIO_FROM = process.env.TWILIO_FROM_NUMBER;
var SITE_URL = process.env.SITE_URL || "";
var TIERS = { free: { daily: 1 }, plus: { daily: 25 }, group: { daily: 100 }, pro: { daily: 500 } };
function consentCopy(fromUsername) {
  const invite = SITE_URL ? ` Or sign up: ${SITE_URL}` : "";
  return `Hi, this is Unassigned. You have a link from ${fromUsername}. Reply Y to receive it. Reply STOP to opt out.${invite} Msg&data rates may apply.`;
}
function dropCopy(fromUsername, link, note) {
  const n = note ? `"${String(note).slice(0, 180)}" ` : "";
  return `Unassigned \u{1F441} from ${fromUsername}: ${n}${link}  \xB7 Reply STOP to opt out`;
}
var toE164 = (raw) => {
...
var json = (statusCode, body) => ({ statusCode, headers: { "Content-Type": "application/json", "Cache-Control": "no-store" }, body: JSON.stringify(body) });
async function sendSms(to, body) {
  ...
  if (!res.ok) {
    let detail = `twilio_http_${res.status}`;
    try {
...
async function getUserByToken(token) {
  ...
  const { data } = await supabase.from("users").select("username, tier, api_token").eq("api_token", token).maybeSingle();
  return data || null;
}
async function reserveSend(username, tier) {
  const limit = TIERS[tier].daily;
  const { data, error } = await supabase.rpc("bump_send_count", { p_username: username, p_limit: limit });
  if (error) return { allowed: false, error };
  const row = Array.isArray(data) ? data[0] : data;
  return { allowed: !!(row && row.allowed), count: row ? row.new_count : 0, limit };
}
exports.handler = async (event) => {
  ...
  if (!user.tier || !TIERS[user.tier]) user.tier = "free";
  let body;
  try {
  ...
  const phone = toE164(body.recipient_phone);
  if (!link) return json(400, { error: "link_required" });
  if (!/^\+\d{8,15}$/.test(phone)) return json(400, { error: "invalid_recipient_phone" });
  const { data: c, error: cErr } = await supabase.from("consent_registry").select("status").eq("phone_number", phone).maybeSingle();
  if (cErr) return json(500, { error: "db_lookup_failed", detail: cErr.message });
  const status = c ? c.status : null;
  if (status === "opted_out") return json(200, { ok: true, delivered: false });
  const reserve = await reserveSend(user.username, user.tier);
  if (reserve.error) return json(500, { error: "db_counter_failed", detail: reserve.error.message });
  if (!reserve.allowed) return json(402, { error: "daily_limit_reached", limit: reserve.limit, tier: user.tier, upgrade: true });
  if (status === "opted_in") {
    try {
      await sendSms(phone, dropCopy(user.username, link, note));
      return json(200, { ok: true, delivered: true, channel: "sms", from: user.username });
    } catch (err) {
      return json(502, { error: "sms_send_failed", detail: err.message });
    }
  }
  const { error: upErr } = await supabase.from("consent_registry").upsert({ phone_number: phone, status: "pending" }, { onConflict: "phone_number" });
  if (upErr) return json(500, { error: "db_upsert_failed", detail: upErr.message });
  const { error: hErr } = await supabase.from("held_links").insert({ phone_number: phone, from_username: user.username, link_url: link, note });
  if (hErr) return json(500, { error: "db_hold_failed", detail: hErr.message });
  try {
    await sendSms(phone, consentCopy(user.username));
  } catch (err) {
    return json(502, { error: "consent_sms_failed", detail: err.message });
  }
  return json(200, { ok: true, delivered: false, awaiting_consent: true, from: user.username });
};
